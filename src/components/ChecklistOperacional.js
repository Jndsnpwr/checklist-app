...
// Estrutura de páginas com emojis
const PAGINAS = [
  { titulo: "Acessos e Área Verde", emoji: "🌳", itens: ["Acessos de entrada do posto", "Acessos de saída do posto", "Jardim/paisagismo", "Canteiros centrais"] },
  { titulo: "Comunicação Visual", emoji: "📢", itens: ["Placa de Preços", "Totem", "Cobertura do posto (foto ampliada)", "Testeira", "Outdoors"] },
  { titulo: "Bombas e Filtros", emoji: "⛽", itens: ["Bombas", "Sumps de bombas", "Lacres de bombas", "Filtros"] },
  { titulo: "Área dos Tanques", emoji: "💧", itens: ["Boca de descarga", "Boca de visita", "Canaletas", "Caixa separadora"] },
  { titulo: "Limpeza e Banheiros", emoji: "🧼", itens: ["Limpeza do pátio", "Banheiros", "Limpeza dos banheiros"] }
];

// Estilos CSS para o novo design
const checklistStyleTag = `
  ...
  .navigation-buttons button:disabled { background: rgba(255, 255, 255, 0.05); color: #666; cursor: not-allowed; }
  .send-button { background: #27ae60; }
  .send-button:hover { background: #2ecc71; }
`;

export default function ChecklistOperacional({ usuarioLogado, onVoltar }) {
  // Lógica de estados e funções (sem alteração)
      ...
      for (let i = 0; i < PAGINAS.length; i++) {
        ...
        for (let j = 0; j < paginaAtual.itens.length; j++) {
          ...
          for (const fotoInfo of fotosDoItem) {
            ...
            const fotoRef = ref(storage, `checklists/operacional/${usuarioLogado.id}/${timestamp}_${i}_${j}.jpg`);
    ...
    <div className="checklist-background">
      <div className="checklist-container">
        ...
        <div className="page-indicator">
          ...
          <p className="page-number">{`Página ${pagina + 1} de ${PAGINAS.length}`}</p>
        </div>
        ...
        <form onSubmit={e => e.preventDefault()}>
          {PAGINAS[pagina].itens.map((item, idx) => (
            <div key={idx} className="item-card">
              ...
              {/* BLOCO DE FUNCIONALIDADES CORRIGIDO E COMPLETO */}
        ...
        <div className="navigation-buttons">
          <button type="button" onClick={handleVoltarPagina} disabled={pagina === 0} className="action-button secondary-button">Voltar</button>
          {pagina === PAGINAS.length - 1 ? (
            ...
            <button type="button" onClick={handleAvancar} className="action-button">Próximo</button>
          )}
        </div>
